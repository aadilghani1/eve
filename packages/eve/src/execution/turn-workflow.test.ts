...
vi.mock("./settle-cancelled-turn-step.js", () => ({
  ...
  settleDeclinedSessionLimitStep: vi.fn(),
}));
...
describe("turnWorkflow", () => {
  ...
  it("terminally completes a session-limit cancellation", async () => {
    ...
    installInbox([]);
    vi.mocked(turnStep).mockResolvedValueOnce({
      action: "session-limit-declined",
      serializedContext: { state: "cancelled" },
      sessionState,
    });
    vi.mocked(settleDeclinedSessionLimitStep).mockResolvedValueOnce({
    ...
    expect(settleDeclinedSessionLimitStep).toHaveBeenCalledWith({
      parentWritable,
      serializedContext: { state: "start" },
      sessionState,
    });
