...
describe("createTurnCancellationControl", () => {
  ...
  it("preserves a session-limit decline as the abort reason", async () => {
    installCancelHook({ payloads: [{ kind: "session-limit-declined" }] });
    ...
    await expect(control!.requested).resolves.toBe("cancel");
    expect(isSessionLimitDecline(control!.signal.reason)).toBe(true);
  });
