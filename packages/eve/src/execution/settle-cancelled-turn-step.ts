...
import { emitCancelledTurn, emitSessionLimitDeclined } from "#harness/cancelled-turn-emission.js";
import {
...
/**
 * Settles a declined session-limit continuation as `turn.cancelled` →
 * `session.completed`.
 */
export async function settleDeclinedSessionLimitStep(
  input: CancelledTurnSettleInput,
): Promise<CancelledTurnSettleResult> {
  "use step";
    ...
    try {
      const scoped = await withContextScope(ctx, session, async (enrichedSession) => {
        ...
        return {
          result:
            boundary === "completed"
              ? await emitSessionLimitDeclined(emit, emissionState)
