...
import { getSessionTokenBudgetWindowUsage } from "#harness/turn-tag-state.js";
import type { HarnessSession } from "#harness/types.js";

/**
 * Computes the session token limits a delegated child inherits from its parent:
 * the current budget window's remaining quota split evenly across the batch's
 * delegated calls, per axis, at dispatch time. `false` marks an axis with no
 * inherited cap.
 *
 ...
 * A granted continuation resets the window used here without erasing lifetime
 * usage.
 */
